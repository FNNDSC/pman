...
PmanHandler = Flask(__name__)
PmanHandler.url_map.strict_slashes = False
restful_api = Api(PmanHandler, prefix="/api/v1/cmd")
...
class pman(object):
    ...
    def __init__(self, **kwargs):
        ...
        # Enable arguments to be available to the entire class
        self.class_arguments = kwargs
    ...
    def start(self):
        """
            ...
            * Instantiate Flask Restful Server
                **  Process input from external requests. Thread out
            ...
            * Instantiate a job poller thread
                **  'poller' examines the internal DB entries and regularly
                    ...
                    are still running.
        """
        self.dp.qprint('Starting Listener threads')
        ...
        self.class_arguments["service_name"] = self.class_arguments['name']
        self.class_arguments["within"] = self
        self.class_arguments.pop('name', None)

        restful_api.add_resource(Listener, '/', resource_class_kwargs=self.class_arguments)
        PmanHandler.run(host=self.str_IP, port=self.str_port)
...
class FileIO(threading.Thread):
    ...
    def directiveCmd_process(self, d_request):
        ...
        d_process = self.within.processPOST(
            request=d_request,
            ret=d_request
        )
...
class Listener(Resource):
    ...
    def post(self):
        ...
        """
        length_request = int(request.headers['content-length'])
        self.worker_id = 1
        ...
        if length_request:
            ...
            content_request = (request.environ['wsgi.input'].read(length_request))
            ...
            self.dp.qprint("***********************************************")
            self.dp.qprint("len = %d" % length_request)
            self.dp.qprint("***********************************************")
            self.dp.qprint(Colors.CYAN + "%s\n" % (content_request.decode()) + Colors.YELLOW)
            self.dp.qprint("***********************************************" + Colors.NO_COLOUR)

            l_raw           = content_request.decode().split('\n')
            FORMtype        = l_raw[0].split('/')[0]

            self.dp.qprint('Request = ...')
            self.dp.qprint(l_raw)
            REST_header             = str(request.headers)
            REST_verb               = str(request.method)
            ...
            if (not self.b_tokenAuth) or self.authorizeRequest():
    ...
    def authorizeRequest(self):
        ...
        if self.within.str_tokenPath != '':
            self.config = configparser.ConfigParser()
            self.config.read(self.within.str_tokenPath)
        AUTH_TOKEN = str(self.config.get('AUTH TOKENS', 'token'))

        # Authenticate user request if the server is started with the b_tokenAuth private flag set to true
        print('Authorizing client request...')
        requestAuth_headers = request.headers.get('Authorization')

        if requestAuth_headers:
    ...
    def t_run_process_container(self, **kwargs):
        """
    ...
    def t_run_process_openshift(self, **kwargs):
        """
        ...
        if d_meta:
            ...
            # Currently it is assumed that incoming and outgoing dir will always be the last two arg
        ...
        if b_threaded:
            ...
            try:
                ...
                raise NotImplementedError(
                    "Class `{}` does not implement `{}`".format(pman.__class__.__name__, str_method))
